#include "map.h"
#include "checkpoint.h"  // for ActionCheckpoint and CheckpointType
#include "zipper.h"
...
void Map::debugPrintFloorObjects() const {
    std::cout << "------ Floor Objects ------" << std::endl;
    for (size_t i = 0; i < floorObjects.size(); i++) {
        const FloorObjectPtr &obj = floorObjects[i];
        std::cout << "Object " << i << ": " << obj->name()
                  << " at (" << obj->getTopLeft().x << ", " << obj->getTopLeft().y << ") "
                  << "with hitbox (" << obj->getHitbox().left << ", " 
                  << obj->getHitbox().top << ", " 
                  << obj->getHitbox().width << " x " << obj->getHitbox().height << ")"
                  << std::endl;
    }
    std::cout << "---- Special Floor Objects ----" << std::endl;
    for (size_t i = 0; i < specialFloorObjects.size(); i++) {
        const FloorObjectPtr &obj = specialFloorObjects[i];
        std::cout << "Special Object " << i << ": " << obj->name()
                  << " at (" << obj->getTopLeft().x << ", " << obj->getTopLeft().y << ") "
                  << "with hitbox (" << obj->getHitbox().left << ", " 
                  << obj->getHitbox().top << ", " 
                  << obj->getHitbox().width << " x " << obj->getHitbox().height << ")"
...
bool Map::loadCourse(const std::string &course) {
    ...
    if (!inCourse.is_open() || !inSkyBack.is_open() || !inSkyFront.is_open() ||
        ...
        std::cerr << "ERROR: Can't find files for '" << course << "'" << std::endl;
        return false;
    }
    // Free resources
    inCourse.close();
    ...
    instance.stretchedGoal = sf::FloatRect(meta_x / MAP_ASSETS_WIDTH,
                                           meta_y / MAP_ASSETS_HEIGHT,
                                           meta_w / MAP_ASSETS_WIDTH,
                                           meta_h / MAP_ASSETS_HEIGHT);
    inObjFile >> meta_x >> meta_y >> meta_w >> meta_h;
    instance.centeredGoal = sf::FloatRect(meta_x / MAP_ASSETS_WIDTH,
                                          meta_y / MAP_ASSETS_HEIGHT,
                                          meta_w / MAP_ASSETS_WIDTH,
                                          meta_h / MAP_ASSETS_HEIGHT);
    ...
    for (int i = 0; i < numObjects; i++) {
        ...
        FloorObjectOrientation orientation = static_cast<FloorObjectOrientation>(orientId);
        ...
        switch (FloorObjectType(typeId)) {
            ...
            case FloorObjectType::RAMP_VERTICAL:
                ...
                break;
            // New checkpoint case (assuming FloorObjectType::CHECKPOINT is defined as 7)
            case FloorObjectType::CHECKPOINT: {
                CheckpointType cpType;
                switch (orientId) {
                    case 0: cpType = CheckpointType::TURN_LEFT;    break;
                    case 1: cpType = CheckpointType::TURN_RIGHT;   break;
                    case 2: cpType = CheckpointType::SHIFT_UP;     break;
                    case 3: cpType = CheckpointType::SHIFT_DOWN;   break;
                    case 4: cpType = CheckpointType::GAS_PEDAL;      break;
                    case 5: cpType = CheckpointType::BRAKE_PEDAL;    break;
                    default:
                        cpType = CheckpointType::GAS_PEDAL;
                        std::cerr << "Warning: unknown checkpoint orientation " 
                                << orientId << " in object " << i << std::endl;
                        break;
                }
                int assetIndex;
                if (size.x == 64 && size.y == 8){
                    assetIndex = 0;
                } else if (size.x == 8 && size.y == 64){
                    assetIndex = 1;
                } else if (size.x == 96 && size.y == 8){
                    assetIndex = 2;
                } else if (size.x == 8 && size.y == 96){
                    assetIndex = 3;
                } else if (size.x == 128 && size.y == 8){
                    assetIndex = 4;
                } else if (size.x == 8 && size.y == 128){
                    assetIndex = 5;
                } else if (size.x == 16 && size.y == 16){
                    assetIndex = 6;
                }

                // Create a checkpoint using the new Checkpoint class.
                ptr = FloorObjectPtr(new Checkpoint(pos, FloorObjectOrientation::UP, cpType, assetIndex));
                instance.specialFloorObjects.push_back(ptr);
                break;
            }            
            default:
                std::cerr << "ERROR: Invalid floor object type (" << typeId
                          << ") in object " << i << std::endl;
                break;
        }
        // Add the object to the floor objects list
    ...
    // At the end of Map::loadCourse(), after loading all objects:
    //instance.debugPrintFloorObjects();
    ...
    for (int i = 0; i < numObjects; i++) {
        ...
        // Add it to the map's wall objects
...
void Map::reactivateQuestionPanels() {
    for (FloorObjectPtr &object : instance.specialFloorObjects) {
        ...
        Checkpoint *ptr_cp = dynamic_cast<Checkpoint *>(object.get());
        if (ptr_cp) {
            ptr_cp->setState(FloorObjectState::ACTIVE);
        }
    }
}
