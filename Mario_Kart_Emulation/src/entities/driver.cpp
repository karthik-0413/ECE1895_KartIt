...
void Driver::applyMushroom() {
    ...
    speedForward = vehicle->maxSpeedUpLinearSpeed * 0.4f;
...
bool Driver::isWithinCheckpointRange(const sf::Vector2f &pos) {
    // Example for one checkpoint hitbox. You can extend this with an array of hitboxes if needed.
    sf::FloatRect checkpoint1(0.554688f, 0.0234375f, 0.015625f, 0.015625f);
    sf::FloatRect checkpoint2(0.554688f, 0.0390625f, 0.015625f, 0.015625f);
    sf::FloatRect checkpoint3(0.554688f, 0.0546875f, 0.015625f, 0.015625f);
    
    return checkpoint1.contains(pos) ||
           checkpoint2.contains(pos) ||
           checkpoint3.contains(pos);
}
...
void Driver::update(const sf::Time &deltaTime) {
    ...
    if (playerSelection) {
        maxLinearSpeed = maxLinearSpeed + (COIN_SPEED * 10 * coins);
    }
    else
    {
        maxLinearSpeed = maxLinearSpeed * 0.75;
    }
