    ...
    switch (currentState) {
        ...
        case RaceState::RACING: {
            ...
            drivers[(unsigned int)selectedPlayer]->controlType =
                DriverControlType::AI_GRADIENT;
            drivers[(unsigned int)selectedPlayer]->setPlayerSelection(true);
            int ret;
            if (CIRCUIT_ASSET_NAMES[i] == "assets/circuit/donut_plains_1"){
                ret = std::system("start /B F:\\home\\kart-it\\serial_sender\\serial_DP.exe");
            } else if (CIRCUIT_ASSET_NAMES[i] == "assets/circuit/mario_circuit_2"){
                ret = std::system("start /B F:\\home\\kart-it\\serial_sender\\serial_MC.exe");
            } else if (CIRCUIT_ASSET_NAMES[i] == "assets/circuit/ghost_valley_1"){
                ret = std::system("start /B F:\\home\\kart-it\\serial_sender\\serial_GV.exe");
            } else if (CIRCUIT_ASSET_NAMES[i] == "assets/circuit/bowser_castle_1"){
                ret = std::system("start /B F:\\home\\kart-it\\serial_sender\\serial_BC.exe");
            } else if (CIRCUIT_ASSET_NAMES[i] == "assets/circuit/rainbow_road"){
                ret = std::system("start /B F:\\home\\kart-it\\serial_sender\\serial_RR.exe");
            }
