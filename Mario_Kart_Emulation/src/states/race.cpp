...
void StateRace::handleEvent(const sf::Event& event) {
    ...
    // Accelerate
    if (Input::pressed(Key::ACCELERATE, event)) {
        // Assuming that 'player' is a member variable of type DriverPtr
        // representing driver 0 (the player's driver)
        if (player->getPlayerSelection()) {
            // Call applyMushroom() on driver 0
            std::cout << "Accelerate key pressed for driver 0; applying mushroom boost." << std::endl;
            player->addCoin(1);
            if (getGasCheckpoint())
            {
                player->applyMushroom();
                setGasCheckpoint(false);
            }
    ...
    // Brake
    if (Input::pressed(Key::BRAKE, event)) {
        // Assuming that 'player' is a member variable of type DriverPtr
        if (player->getPlayerSelection()) {
            // Call applyMushroom() on driver 0
            std::cout << "Brake key pressed for driver 0; applying hit." << std::endl;
            player->applyHit();
        }
